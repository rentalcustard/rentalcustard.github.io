...
Very closely related to the Single Responsibility Principle<sup>[1](#footnote-1)</sup> this is the principle that an object should normally avoid making decisions about what to do based on the state of another object. Here, as ever, I am heavily indebted to Steve Freeman and Nat Pryce<sup>[2](#footnote-2)</sup>, and I see no better way of illustrating this principle than to quote from them, translating from Java to Ruby:
...
Observe that the simplest form of a unit test runs something like this<sup>[5](#footnote-5)</sup>:
...
Here, we arrange our unit under test so that it is ready for us to exercise the behaviour with which we are interested (line 2), act upon it (line 3), and finally make an assertion based on the unit's state<sup>[6](#footnote-6)</sup>.
